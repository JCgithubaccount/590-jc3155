...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct  5 10:37:34 2021
...
max_iter=3000        #MAX NUMBER OF ITERATION
...
def model(x,p):
    ...
    if(model_type=="ANN"):
        sub_matrices = extract_submatrices(p)
        out = ANN(x, sub_matrices)
    return  out 
...
def loss(p,index_2_use):
    ...
    if model_type != 'ANN':
        loss=np.mean(errors**2.0)
    else:
        if regularization == 'L1':
            loss=np.mean(errors**2.0)+GAMMA_L1*sum([abs(i) for i in p]) 
        elif regularization == 'L2':
            loss=np.mean(errors**2.0)+GAMMA_L2*sum([i**2 for i in p])
        else: loss=np.mean(errors**2.0)
    return loss
